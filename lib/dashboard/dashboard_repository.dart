...
class DashboardRepository {
  ...
  Future<VendorDashboardData> loadVendorData(String vendorId) async {
    ...
    // Debug: log runtime types to help diagnose parsing errors
    debugPrint('Vendor shipments response runtimeType=${responses[1].runtimeType}');
    if (responses[1] is List && (responses[1] as List).isNotEmpty) {
      debugPrint('Vendor shipments first element type=${(responses[1] as List)[0].runtimeType}');
    }
  ...
  Future<DeliveryDashboardData> loadDeliveryData(String userId) async {
    ...
    // Debug: log types returned from Supabase for delivery flows
    debugPrint('assignedRows runtimeType=${assignedRows.runtimeType}');
    if (assignedRows is List && assignedRows.isNotEmpty) {
      debugPrint('assignedRows first element type=${assignedRows[0].runtimeType}');
    }
    debugPrint('marketplaceRows runtimeType=${marketplaceRows.runtimeType}');
    if (marketplaceRows is List && marketplaceRows.isNotEmpty) {
      debugPrint('marketplaceRows first element type=${marketplaceRows[0].runtimeType}');
    }
